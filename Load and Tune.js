 ...
 createCSSline("#messange",["color: " + customMap.get("theme-text-color"),
  ...
  document.getElementById("start").onclick = function() { connection.content.begin() }
  ...
  var canvas = new Canvas(canvas_width, canvas_width, 40)
   canvas.save()
  // canvas.changeMarkup()
  ...
  connection.content.body.append(palette.body)
    ...
    connection.content.body.append(panel.body)
      function ColorChoiceEnd(e) {
        if(!e.x) { e.changedTouches[0].screenX }
        var color = panel.colorEditor.activeColor
        if(color) {
         if(e.target == color.c) { panel.colorEditor.vote(color.c) }
         else if(e.x < color.l) { panel.colorEditor.delete(color.c) }
        }
      };
  ...
  tools.controls.create("Switch", function() { 
     bookmark_two.closeBook(); tools.mode.start();
     if(executor.alt) { document.getElementsByTagName("Tools_bu")[5].click() }
   })
 ...
 document.getElementById("start").addResizeMap(function(e) {
  ...
  var prevent_touch = customMap.get("touch-stop", true)
  document.addEventListener("touchstart",function(e) {
    executor.down(e)
    if(prevent_touch) { e.preventDefault(); return false }
      })
  ...
  document.addEventListener("touchend", function(e) {
    executor.up(e); ColorChoiceEnd(e);
    if(prevent_touch) { e.preventDefault(); return false }
    })
  document.addEventListener("touchcansel", function(e) {
   ...
   window.addEventListener("load", function() {
      ...
      var sizes = customMap.get("remember-canvas", true)
