...
class Tools {
 constructor(size) {
  ...
  var ctx = canv.getContext("2d")
   ...
   canv.clickReation(function(e, research) { that.mode.addPoint(research.x,research.y) }, ResearchLayers)
     }
   },
 ...
 createCSSline(".Tools_bu:active", ["border-bottom: 2px solid"])
  ...
  this.controlBrush = function(n) {
   ...
   if(elements[that.content + n]) {  exe(that.content+n) }
    ...
    that.switch(0)
    that.element.clickReaction(function(e, research) {
     if(research.y > that.element.offsetHeight * 4/5) {
    ...
    createCSSline(".toolsInfo", ["font-family: ui-serif", "font-style: oblique",
                                 ...
                                 "font-size: " + 120 * size + "%",
                                 "color: " + customMap.get("global-text-color")])
    that.info.head.innerHTML = `<br> You also able to: `
 
     var cstm = that.controls.create("Customize Paint", function() { window.open("CustomizeMap.html") }, true)
     cstm.style.marginLeft = 0
     that.info.head.append(cstm);
    
     var cstm = that.controls.create("Push Extension", function() { window.open("ExtensionsMap.html") }, true)
     cstm.style.marginLeft = 0
     that.info.head.append(cstm);
  
     that.info.head.append(document.createElement("br"));
     that.info.head.append("Don't forget to visit");
