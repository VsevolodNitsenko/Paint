    ...
    begin: function() {
     ...
     styleArray_normal: [
	       ...
	       { name: "start-button-color-active", value: "#efd6be" },
               ...
               { name: "theme-name", value: "September 1st" },
 ...
 class CustomizeMap {
   ...
   this.view = function(items) { 
   var map = []
   for(var i = 0; i < items.length; i++) {
    var res = that.get(items[i].name)
    if(res == null) { res = items[i].value }
    map.push({ name: items[i].name, value: res })
      }
    return map
    }
   ...
   if(innerHeight <= 850 && !items) { items = connection.styleArray_small; }
   else if(innerHeight <= 1600 && !items) { items = connection.styleArray_normal;  }
   else if(!items) { items = connection.styleArray_large; }
   that.map = that.view(items)
   var version = localStorage.getItem("Paint/version/")
   if(!version) {
      that.map = that.view(items)
      console.warn("Welcome to Paint!")
     } 
   else if( (eval(version) <  eval(connection.version)) ) { 
           var map = that.view(items)
           that.restoreDefault()
  ...
  };  var customMap = new CustomizeMap(0, "Paint/custom-map/")