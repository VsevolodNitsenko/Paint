 ...
 document.addEventListener("mouseup", function(e) { 
   var events = EventBase.getClass("click")
   for(var i = 0; i < events.length; i++) { events[i](e.x, e.y) }
   })
  document.addEventListener("touchend", function(e) {
   var events = EventBase.getClass("click")
   for(var i = 0; i < events.length; i++) { events[i](e.changedTouches[0].pageX, e.changedTouches[0].pageY) }
  })
    HTMLElement.prototype.clickReaction = function(outcome, research, imediately) {
      var element = this
      EventBase.setItem("click", function(x, y) {
        var rect = element.getBoundingClientRect()
        var onwidth = (x > rect.left && x < rect.left+element.offsetWidth)
        var onheight = (y > rect.top && y < rect.top+element.offsetHeight)
        if(onwidth && onheight) { element.takeResearch() }
          })
       var element = this
       element.clickAction = null
       element.takeResearch = function() {
        var e = element.clickAction
        if(e) {
         if(element.research) { element.outcome(e, element.research(e)) } else { element.outcome(e) }
           }; element.clickAction = null;
         }  
      this.addEventListener("mousedown", function(event) {
        element.research = research
        element.outcome = outcome
        element.clickAction = event
      })
     ...
     if(imediately) { element.takeResearch(); }
   };
