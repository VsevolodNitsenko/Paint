class Canvas {
  constructor(width, height, margin, defaultWidth, defaultHeight) {
  const that = this
  this.ins_margin = margin
  this.active = false
  ...
  createCSSline(".CanvasX", ["background-color: rgba(255, 255, 255, 0.6)",
                            "border: 1px dotted gray",
                            "position: absolute",
                            "margin: " + margin  + "px"])
  createCSSline(".CanvasY", ["border: 1px solid rgba(0,0,0,0)",
                             "position: absolute",
                             "margin: " + margin + "px"])
   ...
   this.changeMarkup = function() {
    ...
    this.save = function() {
      ...
      try { panel.colorEditor.deletedColors = null } catch {}
     };
   ...
   this.ndDraw = function(e) {
     if(that.active) {
      ...
      canv.className = "CanvasX"
      this.element.append(canv);

    this.resizeElement = function(wid, het, DisableCorrection) {
       try {
         createCanvas(wid, het); return true
          } 
        catch {
           try { createCanvas(width, height); return undefined }
           catch { 
            console.error("canvas sizes was not established correctly", width, height)
            createCanvas(that.defaultWidth, that.defaultHeight); return false
              }
           }
      function createCanvas(w, h) {
       ...
       if(DisableCorrection || kw == 0 || kh == 0) { kw = w; kh = h }
       ...
       if(that.element.offsetTop + that.canvas.height < 150) {
        that.element.style.marginTop = (innerHeight-that.element.offsetHeight)/2 + "px"
          }
       if(that.element.offsetLeft + that.canvas.width < 150) {
        that.element.style.marginLeft = (innerWidth-that.element.offsetWidth)/2 + "px"       
           }
       var ctx = that.canvas.getContext("2d")
       ctx.getImageData(0,0,ctx.canvas.width,ctx.canvas.height);
         }
       }; that.resizeElement(width-margin*2, height-margin*2);
     }
  }
