 ...
 createCSSline(".panelBody", ["width: " + width * 2 * size + "px",
  ...
  this.body.addResizeMap(function(e) {
      ...
      that.colors.style.marginTop = e.offsetHeight/(e.offsetWidth/2) * 20 * size + "px"
      that.update()
    })
 ...
 this.update = function() {
  ...
  that.colorEditor = {
    ...
    save: function() {
     var height = (that.body.offsetHeight/(that.body.offsetWidth/2) * 40+(editor.colors.length+1)*26)*size
     if(height < that.body.offsetHeight) {
     ...
     editor.strokeColors()
         } else { mess("It's not enough space") }
      },
    ...
    for(var i = 0; i < editor.colors.length; i++) {
     var color = editor.colors[i]
      var d = document.createElement("div")
       ...
       d.id="t" + color.n
       d.active = false
       d.addEventListener("mousedown", function(e) { that.colorEditor.activeColor = { l: e.x, c: this } })
       d.addEventListener("touchstart", function(e) { 
         that.colorEditor.activeColor = { l: e.touches[0].screenX, c: this } 
         })
